# python-ngrok for DingDing

由于原版ngrok内存泄漏问题严重，再加上已经不再更新，因此基本上无法使用。本项目在[python-ngrok](https://github.com/hauntek/python-ngrok)基础上配置好了钉钉免费内网穿透的配置文件，感谢[python-ngrok](https://github.com/hauntek/python-ngrok)开源代码的无私奉献，配合上钉钉可以实现免费高速的内网穿透。

已经将python代码用pyinstaller打包成了exe，64位Windows系统可以直接运行，可自行修改config配置文件后运行`runexe.bat`，或者手动运行`python-ngrok.exe configname`

以下为[python-ngrok](https://github.com/hauntek/python-ngrok)原README
